...
export default function App() {
  ...
  // ── OAuth fragment handler — runs once on mount ───────────────────────────
  // After Google OAuth, the backend redirects to /#token=<jwt> or /#error=<code>.
  // We read the fragment here, store the JWT in React state (in-memory only),
  // then immediately clear the fragment from the browser URL (security hygiene:
  // the token must not persist in browser history or server logs).
  useEffect(() => {
    ...
    if (token) {
      // Fragment contains a JWT — store in memory and clear from URL
      window.history.replaceState(null, '', window.location.pathname + window.location.search);
      // Fetch user info from the backend to populate authUser
      fetch(API.me, {
        ...
        .then((user) => {
          if (user) {
            ...
            setAuthError('Google sign-in succeeded but session could not be established. Please try again.');
          }
        })
        .catch(() => {
          setAuthError('Network error after Google sign-in. Please try again.');
        });
      return;
    }
    ...
    if (errorCode) {
      // Clear the fragment before setting the error so the URL is clean
      window.history.replaceState(null, '', window.location.pathname + window.location.search);
      const errorMessages = {
        google_cancelled: 'Google sign-in was cancelled. You can try again or use email/password.',
        google_error:     'Google authentication failed. Please try again.',
        email_conflict:   'An account with this Google email already exists and is linked to a different Google account. Please contact support.',
        token_invalid:    'Authentication session expired or was tampered with. Please try again.',
      };
      setAuthError(errorMessages[errorCode] || 'Google authentication failed. Please try again.');
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // Intentionally empty deps: fragment read is a one-time mount-only action
  ...
  function handleGoogleLogin() {
    // Navigate the browser to the backend's /auth/google endpoint.
    // The backend handles everything: state generation, redirect to Google,
    // code exchange, id_token verification, user upsert, JWT issuance.
    // On completion, Google redirects back to /auth/google/callback which
    // redirects here with #token=<jwt> or #error=<code> in the fragment.
